...
$homeAddressRequest = mysqli_query($con, "SELECT street, city, zip, state FROM tab_users WHERE id = $LS->user");
$homeRow = mysqli_fetch_array($homeAddressRequest);
$homeAddress = $homeRow['street'] . ", " . $homeRow['city'] . ", " . $homeRow['state'] . ", " . $homeRow['zip'];
...
$homeGeocode=file_get_contents('http://maps.google.com/maps/api/geocode/json?address=' . $homeAddress . '&sensor=false');
...
$today = date("l");
$destAddressRequest = mysqli_query($con,"(SELECT name AS name1, location AS location1, date AS date1, time AS time1, Type AS type1 FROM tab_event WHERE user = '$loggedInUsername' AND date = curdate() AND time >= curtime())
			UNION (SELECT class, location, day, time, Type FROM tab_class WHERE user = '$loggedInUsername' AND day LIKE '%$today%' AND time >= curtime())
			UNION (SELECT concat(class_name, ' ', exam_name), location, date, time, Type FROM tab_exam WHERE user = '$loggedInUsername' AND date = curdate() AND time >= curtime())
...
echo "<div class='content' style='position: absolute; top: 25%;font-size : 25; left: 38%; width:25%; height:48%; background-color: #60aa65; text-align:center; padding: 5px; border:2px solid; border-radius: 25px' >";
	...
	echo "<br> <a href='http://schedulizer.web.engr.illinois.edu/bus_away.html'>Not at home? Click here!</a>";	
}
//echo $routeJson;
?>